...
from Utils.FlowViz import save_vis_flow_tofile

from Utils.Utils import InputPadder, compute_out_of_boundary_mask
from glob import glob
from GMFlowRGBD.Geometry import forward_backward_consistency_check
...
def create_sintel_submission(model,
    ...
    for dstype in ['clean', 'final']:
        ...
        for test_id in range(len(test_dataset)):
            ...
            results_dict = model(image1, image2, attn_splits_list=attn_splits_list, corr_radius_list=corr_radius_list,
            ...
            if not no_save_flo:
                FrameUtils.writeFlow(output_file, flow)
...
def create_kitti_submission(model,
    ...
    for test_id in range(len(test_dataset)):
        ...
        if save_vis_flow:
            ...
            FrameUtils.writeFlowKITTI(output_filename, flow)
...
def validate_sintel(model,
    ...
    for dstype in ['clean', 'final']:
        ...
        for val_id in range(len(val_dataset)):
            if evaluate_matched_unmatched:
                image1, depth1, image2, depth2, flow_gt, valid, noc_valid = val_dataset[val_id]
            ...
            results_dict = model(image1, depth1, image2, depth2,
                                 attn_splits_list=attn_splits_list,
                                 corr_radius_list=corr_radius_list,
                                 prop_radius_list=prop_radius_list,
                                 )
...
def validate_kitti(model,
    ...
    for val_id in range(len(val_dataset)):
        ...
        if with_speed_metric:
            ...
            if average_over_pixels:
                valid_mask = (flow_gt_speed < 10) * (valid_gt >= 0.5)  # KITTI GT是稀疏的
                if valid_mask.max() > 0:
                    s0_10_list.append(epe[valid_mask].cpu().numpy())
...
def inference_on_dir(model,
                     ...
                     padding_factor=8,
                     inference_size=None,
                     paired_data=False,  # 成对testdata的目录，而不是序列
                     save_flo_flow=False,  # 保存为.flo以进行定量评估
                     attn_splits_list=None,
                     corr_radius_list=None,
                     prop_radius_list=None,
    ...
    for test_id in range(0, len(filenames) - 1, stride):
        ...
        image1 = FrameUtils.read_gen(filenames[test_id])
        image2 = FrameUtils.read_gen(filenames[test_id + 1])
        ...
        if pred_bidir_flow:
            ...
            # save vis flowfrom PIL import Image
import os
import time
import numpy as np
import torch
